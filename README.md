...
[![Plugins](https://img.shields.io/badge/Plugins-18-blue)](.claude-plugin/marketplace.json)
[![Skills](https://img.shields.io/badge/Skills-77-green)](.claude-plugin/marketplace.json)
...
| [moai-cost](./moai-cost/) 🆕 | 원가 분석 — 협력업체 견적서, BOM, 공장별 제품 원가, 프로젝트 원가 비교 | 4 |
...
자연어 요청을 분석하여 18개 도메인 플러그인 중 적합한 스킬로 자동 라우팅합니다. `/project init`으로 워크플로우를 인터뷰하여 **스킬 체인 기반 CLAUDE.md**를 생성하고, `/project catalog`로 설치된 스킬 목록을 조회합니다.
...
| 스킬 | 한글명 | 기능 |
|------|--------|------|
| supplier-quote-cost-analysis | 협력업체 견적 원가 분석 | 견적서 세부 내역 분해, 업체별 단가 비교, target price gap 분석 |
| bom-cost-rollup | BOM 원가 roll-up | 제품별 BOM 원가 계산, 2025/2026 단가·수량·BOM 변경 효과 분해 |
| factory-cost-comparison | 공장 원가 비교 | 제품별·공장별 제조원가 2025/2026 비교, 원가 경쟁력 순위 산출 |
| project-cost-variance | 프로젝트 원가 차이 분석 | 전체 프로젝트 원가 bridge, 제품·공장·협력업체별 증감 기여도 분석 |
