#!/usr/bin/env python3
"""
Google Nano Banana Pro Image Generator (v4.0.0 - Gemini 3 Image Preview)

Google Gemini API (Gemini 3 Image Preview 계열)로 이미지를 생성합니다.
v1.0.x의 Imagen 4 기반 구현을 Gemini 3 Image Preview(`:generateContent`)로 완전 마이그레이션.

[v4.0.0 변경사항 — 2026-04-14]
- 모델 전환: imagen-4.0-* → gemini-3-pro-image-preview / gemini-3.1-flash-image-preview
- 엔드포인트: :predict → :generateContent
- 파라미터: numberOfImages + top-level aspectRatio → imageConfig.aspect_ratio + image_size
...
aspect_ratio (14종 지원):
  ...
  16:9  → 와이드 / 유튜브
  21:9  → 시네마틱
  4:5, 5:4, 4:3, 2:3, 3:2, 1:2, 2:1, 9:21, 3:5 기타 지원
...
# 모델 별칭 매핑 (v4.0: Gemini 3 Image Preview)
...
# Gemini 3 Image Preview 지원 화면비 (14종)
