...
## 11. 에이전트 디렉터 협업 규칙
- 하네스 실행 시작 시 3단계 안내 표시 (목표→재료→검토)
- 사용자의 "방향 설정" 역할 존중 — MoAI가 방향을 임의로 변경하지 않음
- 산출물은 항상 "초안"으로 프레이밍
- 수정 요청 시 즉시 반영, 이유 설명은 간결하게
- 사용자의 판단이 최종 결정 — AI의 평가 결과보다 사용자 피드백 우선
```

**절대 축약 금지**: 위 11개 섹션을 모두 포함해야 한다.
5줄짜리 코어 규칙은 품질 부적합.
...
## 7. 00-moai-core.md에 포함할 §11 에이전트 디렉터 협업 규칙

코어 규칙 템플릿의 10개 섹션에 추가로 §11을 포함한다:
...
이 섹션은 00-moai-core.md 생성 시 §10 성능 규칙 이후에 자동 포함한다.
