# MoAI Core Protocol 12개 파일 인덱스

## 개요
MoAI-Cowork v9.0의 핵심 프로토콜 12개 파일이 설계 문서를 기반으로 완성되었습니다.
...
### 12. verification-protocol.md — 산출물 결정론적 검증 프로토콜
**크기**: 4.5KB | **역할**: 기계적 검증 및 하이브리드 검증 루프

산출물의 품질을 AI 판단에만 의존하지 않고, 기계적으로 확인 가능한 항목을 자동 검증:
- **검증 분류**: 결정론적 검증 (구조, 포맷, 수량, 참조, 메타데이터) vs 확률적 검증 (AI 판단 → evaluation-protocol 위임)
- **공통 체크리스트**: 파일 생성 여부, 최소 줄 수, 언어 일치, 프로필 반영, 초안 표기, 면책 문구
- **유형별 체크리스트**: 전략/분석 문서, 실행 문서, 데이터/수치, 프레젠테이션
- **하이브리드 검증 루프**: 확률적 생성 → 결정론적 검증 → Lint-as-Instruction → 수정 → 재검증 (최대 2회)
- **Lint-as-Instruction**: 검증 실패 항목을 AI의 다음 프롬프트로 주입하여 자동 수정 유도
- **Keep Quality Left 원칙**: 기계가 확인 가능한 것은 기계에게, AI는 판단만
