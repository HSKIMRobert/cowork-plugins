...
### 동기화 대상 (총 19개 지점, v1.3.0 기준)

| 범주 | 경로 | 필드 | 개수 |
|---|---|---|---|
| 마켓플레이스 | `.claude-plugin/marketplace.json` | `metadata.version` | 1 |
| 플러그인 매니페스트 | `<plugin>/.claude-plugin/plugin.json` | `version` | 18 |
...
| `<plugin>/README.md` × 18 | 플러그인 설명·스킬 수·신규 스킬 소개 최신화, `v{X}.{Y}.{Z}` 언급 일관성 |
...
# 5. 모든 플러그인 README 존재 (기대: 19 = 루트 + 18 플러그인)
